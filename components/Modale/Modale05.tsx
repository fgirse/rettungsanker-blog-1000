...
const Modale05: React.FC = () => {
  const [show, setShow] = useState(false);

  useEffect(() => {
    // show the modal on mount (replicates original privacyModal.show())
    setShow(true);
  }, []);
  ...
  return (
    <div id="info-popup" tabIndex={-1} className={`${show ? '' : 'hidden'} overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full`}>
      ...
      <div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 md:p-8">
          ...
          <div className="justify-between items-center pt-0 space-y-4 sm:flex sm:space-y-0">
              ...
              <div className="items-center space-y-4 sm:space-x-4 sm:flex sm:space-y-0">
                  ...
                  <button id="confirm-button" type="button" onClick={confirm} className="py-2 px-4 w-full text-sm font-medium text-center text-white rounded-lg bg-primary-700 sm:w-auto hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Confirm</button>
              </div>
          </div>
      </div>
  </div>
